...
// stake_block_timestamp <= rollup_state_cell's L1 timestamp
#[test]
fn test_success_to_unlock_stake_via_finalize_based_on_timestamp() {
    init_env_log();
    let (_, result) = unlock_stake_via_finalize_based_on_timestamp(
...
// stake_block_timestamp > rollup_state_cell's L1 timestamp
#[test]
fn test_fail_to_unlock_stake_via_finalize_based_on_timestamp() {
    init_env_log();
    let (stake_state_cell, result) = unlock_stake_via_finalize_based_on_timestamp(
...
fn unlock_stake_via_finalize_based_on_timestamp(
    stake_block_timestamp: u64,
    ...
    let stake_state_cell = CellOutput::new_builder()
        .lock(
            Script::new_builder()
                ...
                .args({
                    let timepoint = stake_block_timestamp | (1 << 63);
                    let stake_lock_args = StakeLockArgs::new_builder()
                        .stake_block_number(gw_types::prelude::Pack::pack(&timepoint))
